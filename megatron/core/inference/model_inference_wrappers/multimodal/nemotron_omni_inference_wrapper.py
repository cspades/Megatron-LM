...
def _render_nemotron_vl_video_prompt(
    ...
    For example, two frames sampled at 0 and 1 seconds produce
    ``Frame 1 sampled at 0.00 seconds and frame 2 sampled at 1.00 seconds: <img><image></img>``.
    """
    # Reject metadata that cannot produce deterministic frame timestamps.
    ...
    # Match Nemotron-VL's integer-millisecond timestamp calculation exactly.
    frame_duration_ms = int(1000.0 / float(fps))
    tubelet_text = []
    # Group sampled frames by the vision model's temporal patch width.
    for first_frame in range(0, len(frame_indices), temporal_patch_size):
        descriptions = []
        # Describe each source frame represented by this temporal tubelet.
        ...
        # Emit one compact media slot; the normal expansion loop fills its placeholders.
        media_text = prompt_spec.prefix + prompt_spec.model_token + prompt_spec.suffix
        # Nemotron-VL prepends frame timing text to each tubelet's media slot.
    ...
    # Separate consecutive tubelets using the canonical newline boundary.
    return "\n".join(tubelet_text)
...
def _replace_compact_video_slots(
    ...
    *,
    ...
    For example, ``text + <img><image></img>`` becomes
    ``text + frame-1-timestamp + <img><image></img> + ...``.
    """
    rewritten_tokens = []
    video_index = 0
    ...
    def expand_image_tokens(
        ...
        *,
        ...
        if temporal_expansion:
            # Normalize per-video frame counts to validate one input marker per video.
            ...
            # Example: ``[64, 48]`` means the prompt contains videos with 64 and 48 sampled frames.
        ...
        if temporal_expansion:
            # Input example: ``... <img><image></img> ...`` contains one slot for the video.
            ...
            # Resolve the token boundaries of the original ``<img><image></img>`` slot.
            compact_prefix_tokens = tokenizer.tokenize(prompt_spec.prefix)
            compact_suffix_tokens = tokenizer.tokenize(prompt_spec.suffix)

            # Target example: ``Frame 1 ... and frame 2 ...: <img><image></img>`` per tubelet.
            per_video_tokens = []
            ...
            for video_index, frame_count in enumerate(frame_groups):
                ...
                # Select this video's per-tubelet vision-embedding counts.
                ...
                # Render ``Frame 1 sampled at 0.00 seconds: <img><image></img>`` as token IDs.
            ...
            # Rewrite ``<img><image></img>`` into ``Frame 1 ...: <img><image></img>\n...``.
            ...
            # Verify every tubelet now has exactly one marker awaiting visual expansion.
        ...
        # Expand each ``<image>`` marker into ``[-1, ..., -1]`` vision-embedding positions.
        expanded_tokens = []
        image_masks = []
        image_index = 0
        embedding_offset = 0
        for sample_tokens in tokens:
            ...
            for token in sample_tokens:
                ...
                # Example: ``<image>`` -> 256 ``-1`` positions mapped to 256 vision embeddings.
