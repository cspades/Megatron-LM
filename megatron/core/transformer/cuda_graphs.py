...
class _CudagraphReplayNode(torch.autograd.Function):
    ...
    def backward(ctx, *grads):
        ...
        # Skip param.grad post-processing in Megatron-FSDP.
        # wgrad accumulation is captured in the partial CG.
        if runner.mfsdp_model is not None:
            if runner.mfsdp_cudagraph_wgrad_event is None:
                runner.mfsdp_cudagraph_wgrad_event = torch.cuda.Event()
            runner.mfsdp_cudagraph_wgrad_event.record(torch.cuda.current_stream())
            for param in runner.params_to_backprop:
                param._cudagraph_wgrad_event = runner.mfsdp_cudagraph_wgrad_event
...
class _CudaGraphRunner(torch.nn.Module):
    ...
    def __init__(
        ...
        self.mfsdp_cudagraph_wgrad_event = None
    ...
    def create_bwd_graph(self):
        ...
        with torch.cuda.graph(self.bwd_graph, pool=self.mempool):
            ...
            for param, wgrad in zip(self.params_to_backprop, grad_inputs[n_act_grads:]):
                # If wgrad exists, and TE fused wgrad accumulate is deactivated, install the wgrad.
                if wgrad is not None and not getattr(param, 'grad_added_to_main_grad', False):
                    if self.mfsdp_model is not None:
                        # Capture the Megatron-FSDP gradient accumulation here.
                        # Megatron-FSDP allocates a temporary unsharded storage.
                        param.get_main_grad().copy_(wgrad)
                    else:
