...
def _roll_tensor_packed_seq(
    ...
    for i in range(len(cu_seqlens) - 1):
        ...
        if packed_seq_params.cu_seqlens_q_padded is not None:
            ...
            chunk_size = seq_result.size(sequence_dim) // 2
            ...
            valid_length = cu_seqlens[i + 1] - cu_seqlens[i]
            mask_shape = [1] * seq_result.dim()
            mask_shape[sequence_dim] = positions.numel()
            seq_result = seq_result.masked_fill(
                positions.view(mask_shape) >= valid_length - 1, 0
            )
